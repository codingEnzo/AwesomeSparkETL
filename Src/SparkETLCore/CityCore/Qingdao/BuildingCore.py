...
def realEstateProjectId(data):
    # print(data, inspect.stack()[0][3])
    data = data.asDict()
    df = pd.read_sql(" select RealEstateProjectID as col from ProjectInfoItem where ProjectUUID = '{projectUUID}' limit 1 ".format(
...
def address(data):
    # print(data, inspect.stack()[0][3])
    data = data.asDict()
    df = pd.read_sql("select ProjectAddress as col from ProjectInfoItem where ProjectUUID='{projectUUID}' and ProjectAddress !='' order by RecordTime".format(
                             projectUUID = data['ProjectUUID']),Var.ENGINE)
...
def onTheGroundFloor(data):
    # print(data, inspect.stack()[0][3])
    data = data.asDict()
    df = pd.read_sql("select count(distinct ActualFloor) as col from HouseInfoItem where BuildingUUID='{buildingUUID}' and ActualFloor >= '1'".format(
                             buildingUUID = data['BuildingUUID']),Var.ENGINE)
    data['OnTheGroundFloor'] = str(df.col.values[-1]) if not df.empty else ''
    return Row(**data)
...
def housingCount(data):
    # print(data, inspect.stack()[0][3])
    # data = data.asDict()
    # df = pd.read_sql("select count(distinct HouseUUID) as col from HouseInfoItem where BuildingUUID='{buildingUUID}'".format(
    #                          buildingUUID = data['BuildingUUID']),Var.ENGINE)
...
def floors(data):
    # print(data, inspect.stack()[0][3])
    data = data.asDict()
    df = pd.read_sql( "select count(distinct ActualFloor) as col from HouseInfoItem where BuildingUUID='{buildingUUID}'".format(
                             buildingUUID = data['BuildingUUID']),Var.ENGINE)
...
def buildingStructure(data):
    # print(data, inspect.stack()[0][3])
    data = data.asDict()
    df = pd.read_sql("select distinct BuildingStructure as col from HouseInfoItem where BuildingUUID='{buildingUUID}'".format(
                             buildingUUID = data['BuildingUUID']),Var.ENGINE)
...
def buildingArea(data):
    ...
    # print(data, inspect.stack()[0][3])
    data = data.asDict()
    df = pd.read_sql("select MeasuredBuildingArea,HouseUUID from HouseInfoItem where BuildingUUID='{buildingUUID}' order by RecordTime desc".format(
                             buildingUUID = data['BuildingUUID']),Var.ENGINE)
    if not df.empty:
