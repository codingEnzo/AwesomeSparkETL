...
def buildingId(data):
    ...
    data = data.asDict()
    df = pd.read_sql(con = Var.ENGINE,
                     sql = "select BuildingID as col from BuildingInfoItem where BuildingUUID='{buildingUUID}' order by RecordTime DESC limit 1".format(
...
def districtName(data):
    ...
    data = data.asDict()
    df = pd.read_sql(con = Var.ENGINE,
                     sql = "select DistrictName as col from ProjectInfoItem where ProjectUUID='{projectUUID}' order by RecordTime DESC limit 1".format(
                             projectUUID = data['ProjectUUID']))
...
def address(data):
    ...
    data = data.asDict()
    df = pd.read_sql(con = Var.ENGINE,
                     sql = "select ProjectAddress as col from ProjectInfoItem where ProjectUUID='{projectUUID}' order by RecordTime limit 1".format(
                             projectUUID = data['ProjectUUID']))
...
def houseSalePrice(data):
    data = data.asDict()
    df = pd.read_sql(con=Var.ENGINE,
                     sql = "select BuildingAveragePrice as col from BuildingInfoItem where BuildingUUID = '{buildingUUID} order by RecordTime desc limit 1 '")
