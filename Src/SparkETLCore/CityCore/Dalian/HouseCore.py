...
def districtname(data):
    ...
    districtName = rule.search(
            Meth.cleanName(df.col.values[0]).decode('utf-8'))
    if districtName:
...
def measuredBuildingArea(data):
    data = data.asDict()
    rule = re.compile(u'建筑面积:(.+)㎡')
    area = rule.search(Meth.jsonLoad(data['ExtraJson']).get('ExtraHouseInfoStr', ''))
    if MeasuredBuildingArea:
        data['MeasuredBuildingArea'] = area.group(1)
    return Row(**data)


def measuredInsideOfBuildingArea(data):
    data = data.asDict()
    rule = re.compile(u'套内面积:(.+)㎡')
    area = rule.search(Meth.jsonLoad(data['ExtraJson']).get('ExtraHouseInfoStr', ''))
    if MeasuredBuildingArea:
        data['MeasuredInsideOfBuildingArea'] = area.group(1)
    return Row(**data)
