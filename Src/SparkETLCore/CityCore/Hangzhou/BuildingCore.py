...
def housingCount(data):
    data = data.asDict()
    df = pd.read_sql(con = Var.ENGINE,
                     sql = "select count(HouseUUID) as col from HouseInfoItem where BuildingUUID='{buildingUUID}'".format(
...
def floors(data):
    data = data.asDict()
    df = pd.read_sql(con = Var.ENGINE,
                     sql = "select ExtraBuildingFloor as col from HouseInfoItem where BuildingUUID='{buildingUUID}'".format(
                             buildingUUID = data['BuildingUUID']))
