...
def caseTime(data):
    data = data.asDict()
    data['CaseTime'] = Meth.jsonLoad(data['ExtraJson']).get('ExtraCurTimeStamp', '')
    return Row(**data)
...
def projectName(data):
    ...
    if data['ProjectName']:
        ...
        df = pd.read_sql(con = Var.ENGINE,
                         sql = "select ProjectName as col from ProjectInfoItem where ProjectUUID='{projectUUID}' and ProjectName != '' ".format(
...
def city(data):
    data = data.asDict()
    data['City'] = data.get('City') if 'City' in data else '杭州'
    return Row(**data)
...
def forecastBuildingArea(data):
    data = data.asDict()
    data['ForecastBuildingArea'] = str(data['ForecastBuildingArea']).replace('㎡', '')
    return Row(**data)


def forecastInsideOfBuildingArea(data):
    data = data.asDict()
    data['ForecastInsideOfBuildingArea'] = str(data['ForecastInsideOfBuildingArea']).replace('㎡', '')
    return Row(**data)


def forecastPublicArea(data):
    data = data.asDict()
    data['ForecastPublicArea'] = str(data['ForecastPublicArea']).replace('㎡', '')
    return Row(**data)


def measuredBuildingArea(data):
    data = data.asDict()
    data['MeasuredBuildingArea'] = str(data['MeasuredBuildingArea']).replace('㎡', '')
    return Row(**data)


def measuredInsideOfBuildingArea(data):
    data = data.asDict()
    data['MeasuredInsideOfBuildingArea'] = str(data['MeasuredInsideOfBuildingArea']).replace('㎡', '')
    return Row(**data)


def measuredSharedPublicArea(data):
    data = data.asDict()
    data['MeasuredSharedPublicArea'] = str(data['MeasuredSharedPublicArea']).replace('㎡', '')
    return Row(**data)


def measuredUndergroundArea(data):
    data = data.asDict()
    data['MeasuredUndergroundArea'] = str(data['MeasuredUndergroundArea']).replace('㎡', '')
    return Row(**data)
...
def sourceUrl(data):
    data = data.asDict()
    data['SourceUrl'] = Meth.jsonLoad(data['ExtraJson']).get('ExtraHouseUrl', '')
    return Row(**data)
